...
training_output_path = (
    nb_utils.resolve_config_path("output_rf_detr_train", config) / "20260915_185454"
)
training_runs = [f.name for f in training_output_path.iterdir() if f.is_dir()]
...
for training_run in training_runs:
    ...
    best_model_chkpt_path = (
        training_output_path / training_run / "checkpoint_best_ema.pth"
    )
    print(f"best_model_chkpt_path={best_model_chkpt_path}")

    dataset_path_2d = nb_utils.resolve_config_path("data_path", config) / training_run
    ...
    test_split_sample_ids = prefixes = {
        "_".join(f.stem.split("_")[:2]) for f in png_files
    }
    ...
    metrics_dict = {
        "model_chkpt_path": str(
            best_model_chkpt_path.relative_to(nb_utils.PROJECT_ROOT)
        ),
    ...
    metrics_file = (
        best_model_chkpt_path.parent
        / f"3DTeethSeg_eval_{best_model_chkpt_path.stem}_{timestamp}.json"
    )
    print(f"metrics_file={metrics_file}")
    with open(metrics_file, "w") as f:
        json.dump(metrics_dict, f)

    # run 3DTeethSeg evaluation

    obj_files = list(dataset_path_3d.rglob("*.obj"))

    TLA, TSA, TIR = [], [], []
    for pred_label_dict in tqdm(
        metrics_dict["predictions"], desc="Evaluating predictions"
    ):
        try:
    ...
    # export metrics evaluation metrics
    ...
    eval_metrics_file = (
        best_model_chkpt_path.parent
        / f"3DTeethSeg_eval_{best_model_chkpt_path.stem}_results_{timestamp}.json"
    )
