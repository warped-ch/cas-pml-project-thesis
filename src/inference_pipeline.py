...
class InferencePipeline:
    ...
    def run_inference(self, obj_file: str) -> tuple[Meshes, NDArray[np.uint8]]:
        ...
        images = self.view_projector.render_2d_views_composite_feature_images(mesh)

        # tensor from render_2d_views_composite_feature_images is in (Batch, H, W, C) format
        if isinstance(images, torch.Tensor) and images.dim() == 4:
            print(f"images.shape={images.shape}, images.dim={images.dim()}")
